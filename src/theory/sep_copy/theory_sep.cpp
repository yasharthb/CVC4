 ...
 **
 ** \brief Implementation of the theory of sep_copy_copy.
 **
 ** Implementation of the theory of sep_copy_copy.
 **/

#include "theory/sep_copy_copy/theory_sep_copy_copy.h"
...
#include "options/sep_copy_copy_options.h"
...
#include "theory/sep_copy_copy/theory_sep_copy_copy_rewriter.h"
...
TheorySep::TheorySep(context::Context* c,
    ...
    : Theory(THEORY_SEP, c, u, out, valuation, logicInfo, pnm),
      ...
      d_im(*this, d_state, nullptr, "theory::sep_copy_copy"),
      d_notify(*this),
      d_reduce(u),
...
void TheorySep::declareSepHeap(TypeNode locT, TypeNode dataT)
{
  if (!d_type_ref.isNull())
  {
    ...
    std::stringstream ss;
    ss << "ERROR: cannot declare heap types for sep_copyaration logic more than "
...
bool TheorySep::needsEqualityEngine(EeSetupInfo& esi)
{
  ...
  esi.d_name = "theory::sep_copy_copy::ee";
  return true;
}
...
{
  Debug("sep_copy_copy") << "TheorySep::propagateLit(" << literal << ")" << std::endl;
  // If already in conflict, no more propagation
  if (d_state.isInConflict())
  {
    Debug("sep_copy_copy") << "TheorySep::propagateLit(" << literal
...
{
  Debug("sep_copy_copy") << "TheorySep::explain(" << literal << ")" << std::endl;
...
void TheorySep::postProcessModel( TheoryModel* m ){
  Trace("sep_copy-copy-model") << "Printing model for TheorySep..." << std::endl;
  
  std::vector< Node > sep_copy_copy_children;
  Node m_neq;
  Node m_heap;
  for( std::map< TypeNode, Node >::iterator it = d_base_label.begin(); it != d_base_label.end(); ++it ){
    ...
    Trace("sep_copy-copy-model") << "Model for heap, type = " << it->first << " with data type " << d_loc_to_data_type[it->first] << " : " << std::endl;
    ...
    if( d_label_model[it->second].d_heap_locs_model.empty() ){
      Trace("sep_copy-copy-model") << "  [empty]" << std::endl;
    }else{
      for( unsigned j=0; j<d_label_model[it->second].d_heap_locs_model.size(); j++ ){
        ...
        Trace("sep_copy-copy-model") << " " << l << " -> ";
        if( d_pto_model[l].isNull() ){
          Trace("sep_copy-copy-model") << "_";
          //m->d_comment_str << "_";
          ...
          Trace("sep_copy-copy-model") << d_pto_model[l];
          Node vpto = d_valuation.getModel()->getRepresentative( d_pto_model[l] );
          Assert(vpto.isConst());
          pto_children.push_back( vpto );
        }
        Trace("sep_copy-copy-model") << std::endl;
        sep_copy_copy_children.push_back( NodeManager::currentNM()->mkNode( kind::SEP_PTO, pto_children ) );
      }
    }
    ...
    Trace("sep_copy-copy-model") << "sep_copy_copy.nil = " << vnil << std::endl;
    Trace("sep_copy-copy-model") << std::endl;
    if( sep_copy_copy_children.empty() ){
    ...
    }else if( sep_copy_copy_children.size()==1 ){
      m_heap = sep_copy_copy_children[0];
    }else{
      m_heap = NodeManager::currentNM()->mkNode( kind::SEP_STAR, sep_copy_copy_children );
    }
    m->setHeapModel( m_heap, m_neq );
  }
  Trace("sep_copy-copy-model") << "Finished printing model for TheorySep." << std::endl;
}
...
void TheorySep::presolve() {
  Trace("sep_copy-copy-pp") << "Presolving" << std::endl;
}
...
void TheorySep::reduceFact(TNode atom, bool polarity, TNode fact)
{
  ...
  if (slbl.isNull())
  {
    Trace("sep_copy-copy-lemma-debug")
    ...
    Node lem;
    Trace("sep_copy-copy-lemma-debug") << "...polarity is " << polarity << std::endl;
    if (polarity)
    {
  ...
  Trace("sep_copy-copy-lemma-debug") << "Reducing assertion " << fact << std::endl;
  Node conc;
  ...
  else
  {
    ...
    if (satom.getKind() == SEP_STAR || satom.getKind() == SEP_WAND)
    {
      ...
      else
      {
        ...
        Trace("sep_copy-copy-lemma")
            << "Sep::Lemma: sep_copy_copy.nil not in wand antecedant heap : " << nrlem
            << std::endl;
        d_out->lemma(nrlem);
      }
      ...
      {
        Trace("sep_copy-copy-lemma") << "Sep::Lemma : definition : " << clem << std::endl;
        d_out->lemma(clem);
      }
    ...
    else if (satom.getKind() == SEP_PTO)
    {
      ...
      // note semantics of sep_copy.nil is enforced globally
    }
    else if (satom.getKind() == SEP_EMP)
    {
      ...
      Trace("sep_copy-copy-lemma") << "Sep::Lemma : emp : " << lem << std::endl;
      d_out->lemma(lem);
    }
    else
    {
  ...
  if (!use_polarity)
  {
    ...
    Trace("sep_copy-copy-lemma-debug")
        << "Negated spatial constraint asserted to sep_copy-copy theory: " << fact
        << std::endl;
    Node g = nm->mkSkolem("G", nm->booleanType());
    d_neg_guard_strategy[g].reset(new DecisionStrategySingleton(
        "sep_copy_copy_neg_guard", g, getSatContext(), getValuation()));
    ...
    Trace("sep_copy_copy-lemma-debug")
    ...
    Trace("sep_copy_copy-lemma") << "Sep::Lemma : (neg) reduction : " << lem << std::endl;
    d_out->lemma(lem);
  }
  else
  {
    ...
    Trace("sep_copy_copy-lemma") << "Sep::Lemma : reduction : " << lem << std::endl;
    d_out->lemma(lem);
  }
}
...
void TheorySep::postCheck(Effort level)
{
  ...
  NodeManager* nm = NodeManager::currentNM();
  Trace("sep_copy_copy-process") << "Checking heap at full effort..." << std::endl;
  d_label_model.clear();
  d_tmodel.clear();
  d_pto_model.clear();
  Trace("sep_copy_copy-process") << "---Locations---" << std::endl;
       ...
       ++itt)
  {
    for (const Node& t : itt->second)
    {
      ...
      {
        Trace("sep_copy_copy-process") << "?" << std::endl;
      }
    }
  }
  Trace("sep_copy_copy-process") << "---" << std::endl;
       ...
       ++i)
  {
    ...
    if (use_polarity)
    {
      if (satom.getKind() == SEP_PTO)
      {
        ...
        {
          Trace("sep_copy_copy-process") << "Pto : " << satom[0] << " (" << vv << ") -> "
                               << satom[1] << std::endl;
  ...
  // debug print
  if (Trace.isOn("sep_copy_copy-process"))
  {
    Trace("sep_copy_copy-process") << "--- Current spatial assertions : " << std::endl;
    for( NodeList::const_iterator i = d_spatial_assertions.begin(); i != d_spatial_assertions.end(); ++i ) {
      Node fact = (*i);
      Trace("sep_copy_copy-process") << "  " << fact;
      if (!assert_active[fact])
      {
        Trace("sep_copy_copy-process") << " [inactive]";
      }
      Trace("sep_copy_copy-process") << std::endl;
    }
    Trace("sep_copy_copy-process") << "---" << std::endl;
  }
  if (Trace.isOn("sep_copy_copy-eqc"))
  {
    eq::EqClassesIterator eqcs2_i = eq::EqClassesIterator(d_equalityEngine);
    Trace("sep_copy_copy-eqc") << "EQC:" << std::endl;
    while (!eqcs2_i.isFinished())
    {
      Node eqc = (*eqcs2_i);
      eq::EqClassIterator eqc2_i = eq::EqClassIterator(eqc, d_equalityEngine);
      Trace("sep_copy_copy-eqc") << "Eqc( " << eqc << " ) : { ";
      while (!eqc2_i.isFinished())
      {
        if ((*eqc2_i) != eqc)
        {
          Trace("sep_copy_copy-eqc") << (*eqc2_i) << " ";
        }
        ++eqc2_i;
      }
      Trace("sep_copy_copy-eqc") << " } " << std::endl;
      ++eqcs2_i;
    }
    Trace("sep_copy_copy-eqc") << std::endl;
  }
  ...
  if (options::sep_copy_copyCheckNeg())
  {
    // get active labels
    std::map<Node, bool> active_lbl;
    if (options::sep_copy_copyMinimalRefine())
    {
      for( NodeList::const_iterator i = d_spatial_assertions.begin(); i != d_spatial_assertions.end(); ++i ) {
        Node fact = (*i);
        bool polarity = fact.getKind() != NOT;
        ...
        if( !use_polarity ){
          ...
          if( assert_active[fact] ){
            ...
            {
              Trace("sep_copy_copy-process-debug")
         ...
         ++i)
    {
      ...
      Assert(atom.getKind() == SEP_LABEL);
      TNode slbl = atom[1];
      Trace("sep_copy_copy-process") << "--> Active negated atom : " << satom
                           << ", lbl = " << slbl << std::endl;
      // add refinement lemma
      if (!ContainsKey(d_label_map[satom], slbl))
      {
        Trace("sep_copy_copy-process-debug") << "  no children." << std::endl;
      ...
      Trace("sep_copy_copy-process") << "    Model for " << slbl << " : " << o_b_lbl_mval
                           << std::endl;
      ...
      Node inst = instantiateLabel(satom,
                                   ...
                                   tn,
                                   active_lbl);
      Trace("sep_copy_copy-inst-debug") << "    applied inst : " << inst << std::endl;
      if (inst.isNull())
      {
        inst_success = false;
      }
      ...
      {
        rlems.push_back(lem);
        Trace("sep_copy_copy-process") << "-----> refinement lemma (#" << rlems.size()
                             << ") : " << lem << std::endl;
        Trace("sep_copy_copy-lemma") << "Sep::Lemma : negated star/wand refinement : "
                           << lem << std::endl;
        d_out->lemma(lem);
        addedLemma = true;
      }
      else
      {
        ...
        // base theories
        Trace("sep_copy_copy-process")
            << "*** repeated refinement lemma : " << lem << std::endl;
        Trace("sep_copy_copy-warn")
       ...
       ++it)
  {
    ...
    computeLabelModel(it->second);
    Trace("sep_copy_copy-process-debug") << "Check heap data for " << it->first << " -> "
                               << data_type << std::endl;
    ...
    for (size_t j = 0, hsize = hlmodel.size(); j < hsize; j++)
    {
      ...
      Node l = hlmodel[j][0];
      Trace("sep_copy_copy-process-debug") << "  location : " << l << std::endl;
      if (!d_pto_model[l].isNull())
      {
        Trace("sep_copy_copy-process-debug")
      ...
      if (!ll.isNull())
      {
        Trace("sep_copy_copy-process") << "Must witness label : " << ll
        ...
        Trace("sep_copy_copy-lemma") << "Sep::Lemma : witness finite data-pto : " << lem
                           << std::endl;
        d_out->lemma(lem);
        addedLemma = true;
      }
  ...
  // set up model
  Trace("sep_copy-process-debug") << "...preparing sep_copy model..." << std::endl;
  ...
  Trace("sep_copy-check") << "Sep::check(): " << level
...
void TheorySep::conflict(TNode a, TNode b) {
  Trace("sep_copy-conflict") << "Sep::conflict : " << a << " " << b << std::endl;
...
void TheorySep::ppNotifyAssertions(const std::vector<Node>& assertions) {
  ...
  for (unsigned i = 0; i < assertions.size(); i++) {
    Trace("sep_copy-pp") << "Process assertion : " << assertions[i] << std::endl;
  ...
  if (!d_type_ref.isNull()) {
    if (d_type_data.isNull()) {
      d_type_data = NodeManager::currentNM()->mkSort("_sep_copy_U");
      Trace("sep_copy-type") << "Sep: assume data type " << d_type_data << std::endl;
...
int TheorySep::processAssertion( Node n, std::map< int, std::map< Node, int > >& visited, 
  ...
  if( it==visited[index].end() ){
    Trace("sep_copy-pp-debug") << "process assertion : " << n << ", index = " << index << std::endl;
  ...
  if( !underSpatial && ( !references[index][n].empty() || card>0 ) ){
    ...
    bool add = true;
    if( references_strict[index].find( n )!=references_strict[index].end() ){
      Trace("sep_copy-bound") << "Strict bound found based on " << n << ", index = " << index << std::endl;
      if( bt!=bound_strict ){
...
void TheorySep::registerRefDataTypes(TypeNode tn1, TypeNode tn2, Node atom)
{
  if (!d_type_ref.isNull())
  {
    ...
    {
      std::stringstream ss;
      ss << "ERROR: the sep_copyaration logic heap type has already been set to "
  ...
  // if not declared yet, and we have a sep_copyaration logic constraint, throw
  // an error.
  if (!atom.isNull())
  {
    std::stringstream ss;
    // error, heap not declared
    ss << "ERROR: the type of the sep_copyaration logic heap has not been declared "
          "(e.g. via a declare-heap command), and we have a sep_copyaration logic "
          "constraint "
       << atom << std::endl;
    throw LogicException(ss.str());
  }
  // otherwise set it
  Trace("sep_copy-type") << "Sep: assume location type " << tn1
...
void TheorySep::initializeBounds() {
  if( !d_bounds_init ){
    Trace("sep_copy-bound")  << "Initialize sep_copy bounds..." << std::endl;
    d_bounds_init = true;
    for( std::map< TypeNode, TypeNode >::iterator it = d_loc_to_data_type.begin(); it != d_loc_to_data_type.end(); ++it ){
      TypeNode tn = it->first;
      Trace("sep_copy-bound")  << "Initialize bounds for " << tn << "..." << std::endl;
      unsigned n_emp = 0;
      ...
      Trace("sep_copy-bound") << "Cardinality element size : " << d_card_max[tn] << std::endl;
      Trace("sep_copy-bound") << "Type reference size : " << d_type_references[tn].size() << std::endl;
      Trace("sep_copy-bound") << "Constructing " << n_emp << " cardinality constants." << std::endl;
      for( unsigned r=0; r<n_emp; r++ ){
        Node e = NodeManager::currentNM()->mkSkolem( "e", tn, "cardinality bound element for sep_copylog" );
...
Node TheorySep::getBaseLabel( TypeNode tn ) {
  ...
  if( it==d_base_label.end() ){
    initializeBounds();
    Trace("sep_copy") << "Make base label for " << tn << std::endl;
    std::stringstream ss;
    ss << "__Lb";
    ...
    Trace("sep_copy") << "Make reference bound label for " << tn << std::endl;
    ...
    if( options::sep_copyDisequalC() && tn_is_monotonic ){
    ...
    if( d_bound_kind[tn]!=bound_invalid ){      
      ...
      Trace("sep_copy-bound") << "overall bound for " << d_base_label[tn] << " : " << d_reference_bound_max[tn] << std::endl;

      Node slem = NodeManager::currentNM()->mkNode( kind::SUBSET, d_base_label[tn], d_reference_bound_max[tn] );
      Trace("sep_copy-lemma") << "Sep::Lemma: reference bound for " << tn << " : " << slem << std::endl;
      d_out->lemma( slem );
      //slem = NodeManager::currentNM()->mkNode( kind::SUBSET, d_base_label[tn], d_reference_bound_max[tn] );
      //Trace("sep_copy-lemma") << "Sep::Lemma: base reference bound for " << tn << " : " << slem << std::endl;
      //d_out->lemma( slem );
      ...
      if( d_type_references_card[tn].size()>1 ){
        ...
        for( unsigned i=0; i<(d_type_references_card[tn].size()-1); i++ ){
          ...
          if( !children.empty() ){
            ...
            Trace("sep_copy-lemma") << "Sep::Lemma: symmetry breaking lemma : " << sym_lem << std::endl;
            d_out->lemma( sym_lem );
          }
        }
      }
    ...
    Trace("sep_copy-lemma") << "Sep::Lemma: sep_copy.nil not in base label " << tn << " : " << nrlem << std::endl;
    d_out->lemma( nrlem );
...
Node TheorySep::getLabel( Node atom, int child, Node lbl ) {
  ...
  if( it==d_label_map[atom][lbl].end() ){
    ...
    Node n_lbl = NodeManager::currentNM()->mkSkolem( ss.str(), ltn, "sep_copy label" );
...
Node TheorySep::instantiateLabel( Node n, Node o_lbl, Node lbl, Node lbl_v, std::map< Node, Node >& visited, std::map< Node, Node >& pto_model, 
  ...
  Trace("sep_copy-inst-debug") << "Instantiate label " << n << " " << lbl << " " << lbl_v << std::endl;
  if( options::sep_copyMinimalRefine() && lbl!=o_lbl && active_lbl.find( lbl )!=active_lbl.end() ){
    Trace("sep_copy-inst") << "...do not instantiate " << o_lbl << " since it has an active sublabel " << lbl << std::endl;
    return Node::null();
  }else{
    if( Trace.isOn("sep_copy-inst") ){
      if( n.getKind()==kind::SEP_STAR || n.getKind()==kind::SEP_WAND  || n.getKind()==kind::SEP_PTO || n.getKind()==kind::SEP_EMP ){
        for( unsigned j=0; j<ind; j++ ){ Trace("sep_copy-inst") << "  "; }
        Trace("sep_copy-inst") << n << "[" << lbl << "] :: " << lbl_v << std::endl;
      }
    }
    Assert(n.getKind() != kind::SEP_LABEL);
    if( n.getKind()==kind::SEP_STAR || n.getKind()==kind::SEP_WAND ){
      if( lbl==o_lbl ){
        ...
        for( std::map< int, Node >::iterator itl = d_label_map[n][lbl].begin(); itl != d_label_map[n][lbl].end(); ++itl ){
          ...
          Trace("sep_copy-inst-debug") << "Sublabel " << sub_index << " is " << sub_lbl << std::endl;
          Node lbl_mval;
          ...
          Trace("sep_copy-inst-debug") << "Sublabel value is " << lbl_mval  << std::endl;
          mvals[sub_index] = lbl_mval;
        ...
        if( n.getKind()==kind::SEP_STAR ){
          ...
          if( options::sep_copyChildRefine() ){
      ...
      Trace("sep_copy-inst-debug") << "Is in base (non-instantiating) heap : " << inBaseHeap << " for value ref " << vr << " in " << o_lbl << std::endl;
      std::vector< Node > children;
      if( inBaseHeap ){
        // TODO(project##230): Find a safe type for the singleton operator
        ...
        if( it!=pto_model.end() ){
          ...
          Trace("sep_copy-inst-debug") << "Data for " << vr << " was not specified, do not add condition." << std::endl;
        }
      }
      // TODO(project##230): Find a safe type for the singleton operator
      ...
      Trace("sep_copy-inst-debug") << "Return " << ret << std::endl;
      return ret;
...
void TheorySep::setInactiveAssertionRec( Node fact, std::map< Node, std::vector< Node > >& lbl_to_assertions, std::map< Node, bool >& assert_active ) {
  Trace("sep_copy-process-debug") << "setInactiveAssertionRec::inactive : " << fact << std::endl;
...
void TheorySep::computeLabelModel( Node lbl ) {
  if( !d_label_model[lbl].d_computed ){
    ...
    Trace("sep_copy-process") << "Model value (from valuation) for " << lbl << " : " << v_val << std::endl;
    ...
    for( unsigned j=0; j<d_label_model[lbl].d_heap_locs_model.size(); j++ ){
      ...
      if( itm==d_tmodel.end() ) {
        //Trace("sep_copy-process") << "WARNING: could not find symbolic term in model for " << u << std::endl;
        //Assert( false );
        //tt = u;
        //TypeNode tn = u.getType().getRefConstituentType();
        TypeNode tn = u.getType();
        Trace("sep_copy-process") << "WARNING: could not find symbolic term in model for " << u << ", cref type " << tn << std::endl;
      ...
      Trace("sep_copy-process-debug") << "...model : add " << tt << " for " << u << " in lbl " << lbl << std::endl;
  ...
  if( e2 && ( !e2->d_pto.get().isNull() || e2->d_has_neg_pto.get() ) ){
    ...
    if( !e2->d_pto.get().isNull() ){
      if( !e1->d_pto.get().isNull() ){
        Trace("sep_copy-pto-debug") << "While merging " << t1 << " " << t2 << ", merge pto." << std::endl;
...
void TheorySep::addPto( HeapAssertInfo * ei, Node ei_n, Node p, bool polarity ) {
  Trace("sep_copy-pto") << "Add pto " << p << ", pol = " << polarity << " to eqc " << ei_n << std::endl;
  if( !ei->d_pto.get().isNull() ){
    if( polarity ){
      Trace("sep_copy-pto-debug") << "...eqc " << ei_n << " already has pto " << ei->d_pto.get() << ", merge." << std::endl;
      mergePto( ei->d_pto.get(), p );
    }else{
      Node pb = ei->d_pto.get();
      Trace("sep_copy-pto") << "Process positive/negated pto " << " " << pb << " " << p << std::endl;
      ...
      Trace("sep_copy-pto")  << "Conclusion is " << n_conc << std::endl;
...
void TheorySep::mergePto( Node p1, Node p2 ) {
  Trace("sep_copy-lemma-debug") << "Merge pto " << p1 << " " << p2 << std::endl;
...
void TheorySep::sendLemma( std::vector< Node >& ant, Node conc, InferenceId id, bool infer ) {
  Trace("sep_copy-lemma-debug") << "Do rewrite on inference : " << conc << std::endl;
  conc = Rewriter::rewrite( conc );
  Trace("sep_copy-lemma-debug") << "Got : " << conc << std::endl;
  if( conc!=d_true ){
    if( infer && conc!=d_false ){
      Node ant_n = NodeManager::currentNM()->mkAnd(ant);
      Trace("sep_copy-lemma") << "Sep::Infer: " << conc << " from " << ant_n << " by " << id << std::endl;
      d_im.addPendingFact(conc, id, ant_n);
    }else{
      if( conc==d_false ){
        Trace("sep_copy-lemma") << "Sep::Conflict: " << ant << " by " << id
                           << std::endl;
        d_im.conflictExp(id, ant, nullptr);
      }else{
        Trace("sep_copy-lemma") << "Sep::Lemma: " << conc << " from " << ant
                           << " by " << id << std::endl;
...
}/* CVC4::theory::sep_copy namespace */
